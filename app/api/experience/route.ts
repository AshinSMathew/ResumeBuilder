...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (userDataResult && userDataResult.length > 0) {
      ...
      await db`
        UPDATE user_data 
        SET experience = ${JSON.stringify(experiences)}, 
      ...
      // Insert new record
      await db`
        INSERT INTO user_data (user_id, experience)
        VALUES (${userId}, ${JSON.stringify(experiences)})
      `;
    }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const userDataResult = await db`
      SELECT experience FROM user_data WHERE user_id = ${userId}
    `;
    ...
    return NextResponse.json(
      { experiences: userDataResult[0].experience || [] },
      { status: 200 }
    );
  } catch (error) {
    console.error('Database error:', error);
